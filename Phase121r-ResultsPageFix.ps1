# Phase121r-ResultsPageFix.ps1 — fix Next.js searchParams typing + redeploy
...
function WriteUtf8([string]$p,[string]$t){
  $dir = Split-Path $p -Parent; if ($dir -and -not (Test-Path $dir)) { New-Item -ItemType Directory -Force -Path $dir | Out-Null }
...
# 1) Global text fix: replace bad Promise<Record<string,string>> typing
$fixed = 0
...
# 2) Force a clean, build-safe implementation for the failing page
...
$pageTsx = @(
'// patched by Phase121r — safe typing for searchParams'
...
# 3) Prisma generate (via cmd to avoid PS shims)
...
if ($LASTEXITCODE -ne 0) { Log "WARN vercel exit code $LASTEXITCODE — check log"; }

# Parse a deployment URL (best-effort)
...
Write-Host "`n✅ Done. Full log: $LOG"
