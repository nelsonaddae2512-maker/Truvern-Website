...
type TabKey = "open" | "accepted" | "resolved";
...
export default function VendorFindingsPanel({ vendorId }: { vendorId: number }) {
  ...
  const visibleTabs = useMemo(() => {
    ...
    if (showResolved) base.push({ key: "resolved" as const, label: "Resolved", count: counts.resolved });
    return base;
  }, [counts, showResolved]);

  // If user hides resolved while on resolved tab, snap back.
  useEffect(() => {
    if (!showResolved && tab === "resolved") setTab("open");
  }, [showResolved, tab]);
