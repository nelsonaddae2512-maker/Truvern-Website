...
type SearchParamsPromise = Promise<{ as?: string; tab?: string }>;
...
function riskTone(score: number | null | undefined): string {
  ...
  if (score >= 50) return "bg-amber-500/10 text-amber-300 border-amber-500/60";
...
type TabKey = "overview" | "evidence" | "assessments" | "findings";

function normalizeTab(raw: any): TabKey {
  const s = String(raw ?? "").toLowerCase().trim();
  if (s === "evidence" || s === "assessments" || s === "findings" || s === "overview") return s;
  return "overview";
}
...
export default async function VendorDetailPage({ params, searchParams }: Props) {
  ...
  const tab: TabKey = normalizeTab(sp?.tab);
  ...
  const openIssues = vendor.issues.filter((i) => OPEN_STATUSES.has(String(i.status))).length;

  const criticalOpen = vendor.issues.filter(
    (i) => OPEN_STATUSES.has(String(i.status)) && String(i.severity) === "CRITICAL"
  ).length;

  const acceptedRiskCount = vendor.issues.filter((i) => String(i.status) === "ACCEPTED_RISK").length;

  const summary = vendor.trustProfile?.summary ?? null;

  const tabHref = (t: TabKey) => `/vendors/${vendor.id}${asVendor ? "?as=vendor" : ""}${t !== "overview" ? `${asVendor ? "&" : "?"}tab=${t}` : ""}`;

  const tabs: Array<{ key: TabKey; label: string; count?: number }> = [
    { key: "overview", label: "Overview" },
    { key: "evidence", label: "Evidence", count: evidenceCount },
    { key: "assessments", label: "Assessments", count: assessmentCount },
    { key: "findings", label: "Findings", count: openIssues },
  ];
      ...
      <section className="mb-5 rounded-3xl border border-slate-800 bg-slate-950/80 px-4 py-4 shadow-lg shadow-black/40">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div className="space-y-2">
            <h1 className="text-2xl lg:text-3xl font-semibold text-slate-50">{vendor.name}</h1>

            <div className="flex flex-wrap gap-2">
              {chip(vendor.category)}
              {chip(vendor.tier ? `Tier: ${vendor.tier}` : null)}
              {chip(vendor.criticality ? `Criticality: ${vendor.criticality}` : null)}
            ...
            {asVendor && (
              <div className="mt-3 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 px-4 py-3 text-sm text-emerald-100">
                You’re viewing the <span className="font-semibold">Vendor Portal</span> safe version
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)] gap-5">
        <div className="space-y-5">
          ...
          <section
            id="evidence"
            className={clsx(
              "rounded-3xl border border-slate-800 bg-slate-950/80 px-4 py-4 scroll-mt-24",
              tab !== "overview" && tab !== "evidence" ? "hidden" : ""
            )}
          >
          ...
          {!asVendor && (
            <section
              ...
              className={clsx(
                "rounded-3xl border border-slate-800 bg-slate-950/80 px-4 py-4 scroll-mt-24",
                tab !== "overview" && tab !== "assessments" ? "hidden" : ""
              )}
            >
              <div className="flex items-center justify-between">
                ...
                <Link href="/assessment" className="text-xs text-slate-200/60 hover:text-slate-100">
                  View all ↗
                </Link>
              </div>
              ...
              {vendor.assessments.length === 0 ? (
                ...
                <div className="mt-3 divide-y divide-white/5 overflow-hidden rounded-2xl border border-white/10">
                  {vendor.assessments.slice(0, 6).map((a) => (
                    <Link key={a.id} href={`/assessment/runs/${a.id}`} className="block px-4 py-3 hover:bg-white/5">
          ...
          {/* Findings (deep-link target, redirects out to findings page) */}
          {!asVendor && (
            <section
              id="findings"
              className={clsx(
                "rounded-3xl border border-slate-800 bg-slate-950/80 px-4 py-4 scroll-mt-24",
                tab !== "overview" && tab !== "findings" ? "hidden" : ""
              )}
            >
              <div className="flex items-center justify-between">
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">
              ...
              <p className="mt-2 text-sm text-slate-200/70">
                Review open issues, accepted risk, and resolved findings for this vendor in the Findings inbox.
              </p>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <Link
                  href={`/vendors/${vendor.id}/findings?export=1`}
          ...
          {!asVendor && criticalOpen > 0 && (
            <section className="rounded-3xl border border-rose-500/25 bg-rose-500/10 px-4 py-4">
              ...
              <p className="mt-2 text-sm text-rose-100/90">
                {criticalOpen} critical issue{criticalOpen === 1 ? "" : "s"} open. Prioritize remediation and evidence updates.
              </p>
              <div className="mt-3">
                <Link
                  href={`/vendors/${vendor.id}/findings`}
