# Phase22-Rev5-Lite.ps1 — ultra-stable server-side deploy + health check

$ErrorActionPreference = 'Stop'
$host.ui.RawUI.WindowTitle = "Truvern • StallBuster Lite"
...
$null   = New-Item -Force -ItemType Directory -Path $logs
$stamp  = Get-Date -Format 'yyyyMMdd-HHmmss'
$buildLog  = Join-Path $logs "build-$stamp.log"    # (kept for symmetry)
...
function Run-Native([string]$cmdLine,[string]$logPath,[int]$timeoutSec=420){
  $quoted = $cmdLine + ' ' + ('> "{0}" 2>&1' -f $logPath)
  $p = Start-Process -FilePath 'cmd.exe' -ArgumentList @('/c', $quoted) -NoNewWindow -PassThru
  if (-not $p.WaitForExit($timeoutSec*1000)) {
    try { Stop-Process -Id $p.Id -Force } catch {}
    throw "Timeout after $timeoutSec s: $cmdLine (see $logPath)"
  }
  return $p.ExitCode
}
...
try {
  ...
  # No local build — go straight to server-side deploy (most reliable)
  Write-Host "[1] Deploying server-side (vercel --prod --yes)..." -ForegroundColor Cyan
  $exit = Run-Native "`"$vercel`" --prod --yes" $deployLog 600
  ...
  try {
    ...
    Write-Host ("    /ops/health -> {0}, /api/health -> {1}" -f $h1.StatusCode,$h2.StatusCode) -ForegroundColor Green
  } catch { Write-Host ("    Skipped: {0}" -f $_.Exception.Message) -ForegroundColor DarkYellow }

  Write-Host "`nDone. Logs:" -ForegroundColor Green
  Write-Host "  Build : $buildLog"  -ForegroundColor DarkGray
  Write-Host "  Deploy: $deployLog" -ForegroundColor DarkGray
}
catch {
