...
export async function GET() {
      ...
      await Promise.all(
        (vendors as any[]).map(async (v) => {
          const [o, a, r] = await Promise.all([
            ...
            IssueModel.count({ where: { vendorId: v.id, status: { in: accepted } } }).catch(() => 0),
  ...
  for (const v of vendors as any[]) {
    ...
    rows.push([v.id, v.name ?? "", v.riskScore ?? "", roll.open, roll.accepted, roll.resolved, roll.total]);
  }

  const csv = toCsv(rows);

  // ✅ Phase 325D: log export event (never break export if logging fails)
  try {
    const { userId } = auth();
    const actor = userId
      ? await prisma.user
    ...
    await logActivityEvent({
      organizationId: 1, // NOTE: if you have multi-tenant org resolution here, replace this with the real orgId
      type: "BOARD_REPORT_EXPORTED",
      title: "Board report exported",
      description: "Board report CSV export generated.",
      metadata: {
        kind: "csv",
        route: "/api/board-report/export",
        generatedAt: nowIso,
        vendors: vendorCount,
      ...
      actor: actor
        ? { userId: actor.id, name: actor.name ?? null, email: actor.email ?? null }
        : null,
    });
  } catch {
    // ignore
  ...
  return new NextResponse(csv, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="truvern-board-report-${nowIso.slice(0, 10)}.csv"`,
