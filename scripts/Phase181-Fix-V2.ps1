# ================================
# Phase181-FIX-V2: Evidence Panel & List API
# ================================
Write-Host "===== Phase181-FIX-V2: Evidence Panel & List API START =====" -ForegroundColor Cyan

$root = "C:\Users\MR.NELSON\Downloads\truvern"
Set-Location $root

# Correct vendor file path
$vendorPage = "$root\app\vendors\[id].tsx"
...
if (-Not (Test-Path $vendorPage)) {
    ...
    Write-Host "Vendor file found OK." -ForegroundColor Green
}
...
if ($vendorRaw -notmatch "EvidenceList") {
    ...
    Write-Host "Vendor detail page already patched."
}

# --- Git operations ---
git add app/api/evidence/list
git add app/vendors/[id].tsx
git add components/EvidenceList.tsx

git commit -m "Phase181-FIX-V2: Evidence Panel + List API for vendors/[id].tsx"
git push

Write-Host "===== Phase181-FIX-V2 COMPLETE =====" -ForegroundColor Cyan
