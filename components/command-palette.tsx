...
export default function CommandPalette() {
  ...
  // Filter quick links locally by query
  const filteredQuickLinks = useMemo(() => {
    const trimmed = query.trim().toLowerCase();
    if (!trimmed) return QUICK_LINKS;
    return QUICK_LINKS.filter((link) => {
      const haystack = `${link.label} ${link.description}`.toLowerCase();
  ...
  const items = useMemo(() => {
    type Item =
      | { type: "quick"; link: QuickLink }
      | { type: "vendor"; vendor: VendorResult };

    const result: Item[] = [];

    filteredQuickLinks.forEach((link) => {
      result.push({ type: "quick", link });
    });

    vendorResults.forEach((vendor) => {
      result.push({ type: "vendor", vendor });
    });
  ...
  useEffect(() => {
    // reset index when items or open state change
    if (!open) return;
    setActiveIndex(0);
  ...
  function onKeyDown(e: React.KeyboardEvent<HTMLDivElement>) {
    ...
    if (e.key === "ArrowDown") {
      e.preventDefault();
      setActiveIndex((prev) => (prev + 1) % Math.max(items.length || 1, 1));
      ...
      <div className="w-full max-w-xl rounded-2xl border border-slate-800 bg-slate-950/95 shadow-xl shadow-black/60">
        ...
        <div className="flex items-center gap-2 border-b border-slate-800 px-3 py-2.5">
          ...
          <input
            ...
            placeholder="Search vendors, pages, and actions…"
            value={query}
        ...
        <div className="max-h-80 overflow-y-auto py-2 text-sm">
          {items.length === 0 && !loadingVendors && (
            <p className="px-4 py-6 text-center text-xs text-slate-500">
              No results. Try a vendor name like{" "}
              <span className="font-medium text-slate-300">Rushman Shop</span>.
            </p>
          )}
          ...
          {filteredQuickLinks.length > 0 && (
            <div className="px-2 pb-1 pt-1">
              {["Navigation", "Actions"].map((group) => {
                const groupLinks = filteredQuickLinks.filter(
                  (l) => l.group === group
                );
                  ...
                  <div key={group} className="mb-1">
                    ...
                    {groupLinks.map((link) => {
                      const index = items.findIndex(
                        (it) => it.type === "quick" && it.link.href === link.href
                      );
          ...
          {query.trim().length >= 2 && (
            <div className="px-2 pt-1">
              ...
              {!loadingVendors && vendorResults.length === 0 && (
                <p className="px-4 py-3 text-[11px] text-slate-500">
              ...
              {vendorResults.map((vendor) => {
                const index = items.findIndex(
                  (it) => it.type === "vendor" && it.vendor.id === vendor.id
                );
                const isActive = index === activeIndex;
        ...
        <div className="border-t border-slate-800 px-3 py-2 text-[10px] text-slate-500">
          <span className="mr-3">
            Press <span className="rounded bg-slate-900 px-1">⌘K</span> or{" "}
            <span className="rounded bg-slate-900 px-1">Ctrl+K</span> anywhere
            to open.
          </span>
        </div>
      </div>
